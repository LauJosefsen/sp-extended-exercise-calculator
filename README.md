...
2) Refactor the code by introducing a hierarchy of possible AST **term**s (independent from expression operators).
...
4) The body may contain a nested `SUBCASE` which continue the test case,
   except each `SUBCASE` is independent from its siblings, i.e. the shared test prefix is repeated 
...
Note: [CMakeLists.txt](CMakeLists.txt) contains commands which download and install the `doctest.h` -- the project should work/compile out-of-the-box.
It also registers the target `test_calculator` command as a test, which can be run by `ctest` in the build directory
(**Clion** detects them automatically and displays in the build drop-down-box).
The test produces a report of passed/failed checks and tests when run as a regular binary or via `ctest` utility (part of `cmake`).
